...
EGIT_REPO_URI="git://github.com/arduino/Arduino.git"
EGIT_BRANCH="master"
EGIT_PROJECT="${P}"
...
src_compile() {
	cd "${S}/build"
	ant || echo "ant build failed"
}

src_install() {
	rm -rf "${S}"/build/linux/work/hardware/tools/avrdude[.conf]
	epatch "${FILESDIR}"/arduino-script-${PV}.patch

	mkdir -p "${D}/usr/share" "${D}/usr/bin"

	cp -a "${S}/build/linux/work" "${D}/usr/share/${P}"
